...
import {
  initialMacroQuizData,
  MacroQuiz,
} from "../../components/old/MacroForm/MacroFormData.ts";
import { buttonConfigs } from "../../components/old/ThreeModComponent/buttonConfigData.ts";
import {
  initialMicroQuizData,
  MicroQuiz,
} from "../../components/old/MicroForm/MicroFormData.ts";
...
export const trainingHomeHook = () => {
  ...
  useEffect(() => {
    ...
    const matchedConfig = buttonConfigs.find(
      (config) => config.partialPath === lastSegment,
    );
...
export function trainingHomeHookMacro(): MacroFormContextType {
  const [quizDataMacro, setQuizData] =
    useState<MacroQuiz[]>(initialMacroQuizData);
  ...
  const handleQuantityChangeMacro = useCallback(
    (index: number, delta: number) => {
      setQuizData((prevData) => {
        const updatedData = [...prevData];
        updatedData[index].quantitySelected = Math.max(
          0,
          updatedData[index].quantitySelected + delta,
        );
        return updatedData;
      });
    },
  ...
  const handleInputNumberChangeMacro = useCallback(
    (
      index: number,
      event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>,
    ) => {
...
export const trainingHomeHookMicro = () => {
  const [microQuizData, setMicroQuizData] =
    useState<MicroQuiz[]>(initialMicroQuizData);
    ...
    (categoryIndex: number, subcategoryIndex: number) => {
      setMicroQuizData((prevData) => {
        const updatedSubArg = {
          ...
          isChecked:
            !prevData[categoryIndex].subArgArray[subcategoryIndex].isChecked,
        };

        if (
          updatedSubArg.isChecked ===
          prevData[categoryIndex].subArgArray[subcategoryIndex].isChecked
        ) {
          return prevData;
        }
        ...
        const updatedCategory = {
          ...
          subArgArray: prevData[categoryIndex].subArgArray.map(
            (subArg, index) =>
  ...
  const handleQuantityChangeMicro = useCallback(
    (categoryIndex: number, delta: number, subIndex?: number) => {
      setMicroQuizData((prevData) => {
        if (subIndex !== undefined) {
          const updatedSubArg = {
            ...
            quantitySelected: Math.max(
              0,
              prevData[categoryIndex].subArgArray[subIndex].quantitySelected +
                delta,
            ),
          };

          if (
            updatedSubArg.quantitySelected ===
            prevData[categoryIndex].subArgArray[subIndex].quantitySelected
          ) {
            return prevData;
          }
          ...
          const updatedCategory = {
            ...prevData[categoryIndex],
            subArgArray: prevData[categoryIndex].subArgArray.map(
              (subArg, index) => (index === subIndex ? updatedSubArg : subArg),
            ),
          };
  ...
  const handleInputNumberChangeMicro = useCallback(
    (
      ...
      if (subcategoryIndex !== undefined) {
        ...
        setMicroQuizData((prevData) => {
          const updatedData = [...prevData];
          updatedData[categoryIndex].subArgArray[
            subcategoryIndex
          ].quantitySelected = Math.max(0, parseInt(input, 10) || 0);
          return updatedData;
        });
      }
    },
